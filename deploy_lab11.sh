...
# IP fija del contenedor dentro de red_datos (para resolver "servidor_pgadmin"
# desde /etc/hosts del host y permitir el túnel SSH literal de la práctica).
PGADMIN_STATIC_IP=172.28.5.10
...
# Asegurar que PGADMIN_STATIC_IP exista en .env aunque sea un .env de
# una corrida anterior (versiones previas del script no la generaban).
if ! grep -q '^PGADMIN_STATIC_IP=' .env; then
    echo "PGADMIN_STATIC_IP=172.28.5.10" >> .env
    echo "==> .env actualizado con PGADMIN_STATIC_IP=172.28.5.10"
fi

# Cargar todas las variables de .env al entorno de ESTE script (no solo
# al de docker compose), para poder usarlas más adelante (p. ej. al
# registrar la entrada en /etc/hosts).
set -a
source .env
set +a
...
services:
  ...
  servidor_pgadmin:
    ...
    # Sin "ports": no se publica ningún puerto al host. El acceso es
    # exclusivamente vía IP fija + entrada en /etc/hosts del host,
    # alcanzado a través de un túnel SSH (ver script configurar_hosts.sh).
    expose:
      - "${PGADMIN_LISTEN_PORT}"
    volumes:
    ...
    networks:
      red_datos:
        ipv4_address: ${PGADMIN_STATIC_IP}
    depends_on:
      db_postgres:
...
# ---------------------------------------------------------
# 9) Registrar "servidor_pgadmin" en /etc/hosts del HOST, para que
#    el túnel SSH pueda resolverlo tal como pide la práctica:
#    ssh -L 8080:servidor_pgadmin:80 usuario@ip_servidor
# ---------------------------------------------------------
echo ""
echo "==> Registrando 'servidor_pgadmin' en /etc/hosts del host..."
sudo sed -i '/[[:space:]]servidor_pgadmin$/d' /etc/hosts
echo "${PGADMIN_STATIC_IP}   servidor_pgadmin" | sudo tee -a /etc/hosts > /dev/null
echo "==> Entrada agregada: ${PGADMIN_STATIC_IP} servidor_pgadmin"

echo ""
echo "==> Probando conectividad del host hacia el contenedor pgAdmin..."
if curl -s -o /dev/null -w "" --max-time 5 "http://servidor_pgadmin:${PGADMIN_LISTEN_PORT}"; then
    echo "    OK: el host alcanza a servidor_pgadmin correctamente."
else
    echo "    ADVERTENCIA: el host no pudo alcanzar servidor_pgadmin directamente."
    echo "    Esto puede pasar si la red 'red_datos' con 'internal: true' bloquea"
    echo "    el enrutamiento incluso desde el propio host. Si el túnel SSH falla"
    echo "    más adelante, dímelo para quitar 'internal: true' de red_datos."
fi
...
echo "     ssh -L 8080:servidor_pgadmin:80 usuario@${INTERNAL_IP:-<IP_INTERNA>}"
